...
- [[https://www.youtube.com/watch?v=2nM6DBE0blA&ab_channel=BoostMyTool][How to Fix Error Makefile: *** missing separator. Stop]]
- [[https://www.vantage-ai.com/blog/speed-up-your-python-development-workflow-with-pre-commit-and-makefile][Intro to Makefile]]
- [[https://github.com/azegas/quotes/issues/4][Issue in quotes project]]
...
- [[https://github.com/azegas/quotes/tree/master/tests][Tests example from quotes project]] 
- [[https://docs.djangoproject.com/en/5.0/topics/testing/advanced/][Django docs for testing]] 
- [[https://www.obeythetestinggoat.com/book/praise.forbook.html][Nice resource(online free book about testing]] 
- [[https://ctrlzblog.com/a-beginners-guide-to-unit-testing-in-django?x-host=ctrlzblog.com][A Beginners Guide to Unit Testing in Django]] 
- [[(https://ctrlzblog.com/how-to-test-django-models-with-examples?x-host=ctrlzblog.com][How to Test Django Models (with Examples)]] 
- [[https://github.com/azegas/quotes/issues/4][Issue in quotes project]] 
- [[https://www.youtube.com/watch?v=hA_VxnxCHbo&list=PLbpAWbHbi5rMF2j5n6imm0enrSD9eQUaM&index=3&ab_channel=TheDumbfounds][Testing views tutorial]] 
- [[https://www.youtube.com/watch?v=28zdhLPZ1Zk&list=PLbpAWbHbi5rMF2j5n6imm0enrSD9eQUaM&index=6&ab_channel=TheDumbfounds][Functional tests]] 
...
Run tests with =python manage.py test=. If you want to get more information
abotut the test run you can change the *verbosity*. =python manage.py test
--verbosity 2=.
  
Create =tests= folder in rood directory. Inside of it, create =__init__.py=
file and =test_views.py= file. Create a basic test inside of it:
...
- [[https://docs.github.com/en/actions/quickstart][official docs]]
- https://github.com/azegas/quotes/issues/32
- https://www.honeybadger.io/blog/django-test-github-actions/

Since secret keys are in .env file and =settings.py= takes from it, I need to
store those secrets from =.env= file to github repo for github actions to pick
them up.

Go to =/settings/secrets/actions/new= of your repo and add a SECRET_KEY
variable with a random value.

Create a =.github=folder. Inside of it, create =workflows= folder and inside of
...
1. Isolation: Running each job in its own container provides isolation between
   the jobs. This can be beneficial if one job fails or experiences issues, as
   it won't affect the execution of other jobs.
2. Parallelism: Splitting the workflow into multiple jobs allows for parallel
...
3. Maintenance: Splitting the workflow into smaller, focused files can improve
   ...
   over time.
4. Resource Usage: While running multiple containers may consume more
...
** For security reasons, rename /admin to something else
...
** TODO CRUD functionality with HTMX
